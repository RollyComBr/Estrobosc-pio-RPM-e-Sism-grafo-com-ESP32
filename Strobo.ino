...
enum class Mode {
            ...
            TEST,
            ABOUT };
Mode currentMode = Mode::FREQUENCY;  // Modo inicial do dispositivo
...
const unsigned long encoderDebounce = 7000;       // Tempo de debounce para o encoder em microssegundos
...
const int valMenuOpt = 6;                // Quantidade de opções que possui no menu
...
void updateDisplay() {
  ...
  display.print("Modo: ");
  switch (currentMode) {
    case Mode::FREQUENCY: display.print("Estrobo"); break;
    case Mode::LANTERN: display.print("Lanterna"); break;
    case Mode::RPM: display.print("RPM"); break;
    case Mode::SEISMOGRAPH: display.print("Sismografo"); break;
    case Mode::TEST: display.print("Teste"); break;
    case Mode::ABOUT: display.print("Sobre"); break;
  }
  ...
  if (currentMode == Mode::RPM) {
    ...
    switch (seismoState) {
      ...
      case SEISMO_RESULT:
        display.print("Amp: ");
        display.print(amplitude, 2);
        display.setCursor(0, 26);
        display.print("Hz: ");
        display.print(frequency, 2);
        display.setCursor(0, 38);
        display.print("T: ");
        display.print(vibDuration);
        display.print("ms");
    ...
    display.setCursor(90, 14);
    display.print("S: ");
    display.print(lanternLEDState ? "OK" : "X");
  } else {
  ...
  } else if (phaseSetting && currentMode == Mode::FREQUENCY) {
    display.setCursor(0, 56);
 ...
 */
void IRAM_ATTR IR_sensor_ISR() {
  //pulseCount++;
}

/**
 * @brief Calcula a rotação por minuto (RPM) com base nos pulsos do sensor IR.
 * 
 * A função é chamada continuamente no loop e utiliza uma janela de tempo
 * baseada em microssegundos para calcular a frequência de pulsos. 
 * O valor de RPM é atualizado a cada intervalo definido (ex: 100 ms).
 * 
 * A fórmula usada é:
 *   RPM = (pulsos / intervalo_em_segundos) * 60
 *        = pulsos * (60 / intervalo_s)
 *        = pulsos * (600000 / intervalo_em_microssegundos)
 * 
 * A variável global `rpm` é atualizada com o valor mais recente.
 */
...
void setup() {
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) {
    ...
    while (1)
      ;  // Trava se o display não for encontrado
  }
...
void loop() {
  ...
  if (digitalRead(buttonDouble) == LOW && now - lastButtonTimeDouble > debounceButton) {
    if (!menuActive && currentMode == Mode::FREQUENCY && !phaseSetting) adjustFPM(2.0);
    else if (currentMode == Mode::FREQUENCY && phaseSetting) {
      phase = (phase * 2) % 360;
      updateInterval();
      updateDisplay();
    }
  ...
  if (digitalRead(buttonHalf) == LOW && now - lastButtonTimeHalf > debounceButton) {
    if (!menuActive && currentMode == Mode::FREQUENCY && !phaseSetting) adjustFPM(0.5);
    else if (currentMode == Mode::FREQUENCY && phaseSetting) {
      phase = (phase / 2) % 360;
      updateInterval();
      updateDisplay();
  ...
  if (digitalRead(buttonSet) == LOW && now - lastButtonTimeSet > debounceButton) {
    if (menuActive) {
    ...
    } else if (currentMode == Mode::TEST && !testRunning) {
      runTestSequence();
    } else if (currentMode == Mode::SEISMOGRAPH) {
      if (seismoState == SEISMO_IDLE) {
        ...
        for (int i = 0; i < 50; i++) {
          ...
          float acc = sqrt(event.acceleration.x * event.acceleration.x + event.acceleration.y * event.acceleration.y + event.acceleration.z * event.acceleration.z);
          restAmplitude += acc;
          delay(10);
        }
        restAmplitude /= 50.0;
  ...
  if (encoderValue != lastEncoderValue) {
    ...
    if (menuActive) {
      // Navega pelo menu
      menuIndex = (menuIndex + delta + valMenuOpt) % valMenuOpt;  // O "+valMenuOpt" evita resultados negativos no módulo
    } else if (phaseSetting && currentMode == Mode::FREQUENCY) {
      // Ajusta a fase
      ...
      fpm += delta;
      updateInterval();
    } else if (currentMode == Mode::SEISMOGRAPH && seismoState == SEISMO_CONFIG) {
  ...
  switch (currentMode) {
    ...
    case Mode::RPM:{
      ...
      bool estadoAtual = digitalRead(sensorIRPin);
